...
function BookingFormScreen({ pkg, bookings, addBooking, onBack, onSubmitted }) {
  ...
  const [addons, setAddons] = useState({ makeup: "", photographer: "", mcTier: "", mcVendor: "", mcType: "akad", entertainment: "" });
  const [showCal, setShowCal] = useState(false);
  const [saving, setSaving] = useState(false);
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));
  const setAddon = (k) => (e) => setAddons((a) => ({ ...a, [k]: e.target.value }));
  const valid = form.groomName && form.brideName && form.whatsapp && form.address && form.date;

  const selectedMc = addons.mcTier !== "" ? ADDON_MC[Number(addons.mcTier)] : null;
  const mcPrice = selectedMc
    ? (addons.mcType === "akad" ? selectedMc.priceAkad : addons.mcType === "resepsi" ? selectedMc.priceResepsi : selectedMc.priceAkad + selectedMc.priceResepsi)
    : 0;
  const photographerPrice = ADDON_PHOTOGRAPHER.extra.names.includes(addons.photographer) ? ADDON_PHOTOGRAPHER.extra.price : 0;
  const entertainmentPrice = addons.entertainment === "akustik" ? ADDON_ENTERTAINMENT.akustik : addons.entertainment === "orgen" ? ADDON_ENTERTAINMENT.orgenTunggal : 0;
  const addonsTotal = mcPrice + photographerPrice + entertainmentPrice;
  ...
  const submit = async (e) => {
    ...
    const booking = {
      ...
      price: grandTotal, packagePrice: pkg.price, bookingPay: 2000000, pelunasan: 0, guests: form.guests, notes: form.notes.trim(),
      addons: {
        makeup: addons.makeup || null,
        photographer: addons.photographer || null,
        photographerPrice,
        mc: selectedMc ? { vendor: addons.mcVendor || null, priceAkad: selectedMc.priceAkad, priceResepsi: selectedMc.priceResepsi, type: addons.mcType, price: mcPrice } : null,
        entertainment: addons.entertainment || null,
        entertainmentPrice,
        addonsTotal,
      },
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 -mt-4 pb-10">
        <div className="rounded-2xl p-3 mb-4 flex items-center gap-3 effect-3d-card" style={{ backgroundColor: C.paper, border: `1px solid ${C.line}` }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            {addonsTotal > 0 && <p className="font-body text-xs font-semibold" style={{ color: C.orange }}>Total dengan tambahan: {fmtIDR(grandTotal)}</p>}
          </div>
        ...
        <form onSubmit={submit} className="rounded-2xl p-5 effect-3d-card" style={{ backgroundColor: C.paper, border: `1px solid ${C.line}` }}>
          ...
          {pkg.makeupOptions && pkg.makeupOptions.length > 0 && (
            <Field label="Pilih Akun Makeup (termasuk dalam paket)">
              <select style={inputStyle} value={addons.makeup} onChange={setAddon("makeup")}>
                <option value="">Belum menentukan / diskusi dengan admin</option>
                {pkg.makeupOptions.map((m) => <option key={m} value={m}>@{m}</option>)}
              </select>
            </Field>
          )}

          <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: C.bg }}>
            <p className="font-body text-xs font-semibold mb-3 flex items-center gap-1.5" style={{ color: C.ink }}>
              <Icon name="Plus" size={13} color={C.blueDeep} /> Paket Tambahan (Opsional)
            </p>

            <Field label="Fotografer">
              <select style={inputStyle} value={addons.photographer} onChange={setAddon("photographer")}>
                <option value="">Tidak pakai</option>
                <optgroup label="Standar">
                  {ADDON_PHOTOGRAPHER.names.map((n) => <option key={n} value={n}>@{n}</option>)}
                </optgroup>
                <optgroup label={`Ekstra kelas (+${fmtIDR(ADDON_PHOTOGRAPHER.extra.price)})`}>
                  {ADDON_PHOTOGRAPHER.extra.names.map((n) => <option key={n} value={n}>@{n}</option>)}
                </optgroup>
              </select>
            </Field>

            <Field label="Master of Ceremony (MC)">
              <select
                style={inputStyle}
                value={addons.mcTier}
                onChange={(e) => setAddons((a) => ({ ...a, mcTier: e.target.value, mcVendor: "" }))}
              >
                <option value="">Tidak pakai</option>
                {ADDON_MC.map((tier, i) => (
                  <option key={i} value={i}>
                    Paket {i + 1} — Akad {fmtIDR(tier.priceAkad)} / Resepsi {fmtIDR(tier.priceResepsi)}
                  </option>
                ))}
              </select>
            </Field>
            ...
            {selectedMc && (
              <>
                <Field label="Nama MC">
                  <select style={inputStyle} value={addons.mcVendor} onChange={setAddon("mcVendor")}>
                    <option value="">Belum menentukan / diskusi dengan admin</option>
                    {selectedMc.names.map((n) => <option key={n} value={n}>@{n}</option>)}
                  </select>
                </Field>
                <Field label="Sesi MC">
                  <select style={inputStyle} value={addons.mcType} onChange={setAddon("mcType")}>
                    <option value="akad">Akad saja ({fmtIDR(selectedMc.priceAkad)})</option>
                    <option value="resepsi">Resepsi saja ({fmtIDR(selectedMc.priceResepsi)})</option>
                    <option value="both">Akad + Resepsi ({fmtIDR(selectedMc.priceAkad + selectedMc.priceResepsi)})</option>
                  </select>
                </Field>
              </>
            )}
            ...
            <Field label="Entertainment">
              <select style={inputStyle} value={addons.entertainment} onChange={setAddon("entertainment")}>
                <option value="">Tidak pakai</option>
                <option value="akustik">Akustik ({fmtIDR(ADDON_ENTERTAINMENT.akustik)})</option>
                <option value="orgen">Orgen Tunggal ({fmtIDR(ADDON_ENTERTAINMENT.orgenTunggal)})</option>
              </select>
            </Field>
          </div>

          {addonsTotal > 0 && (
            <div className="rounded-xl p-4 mb-4 font-body text-xs space-y-1" style={{ backgroundColor: C.orangeSoft }}>
              <div className="flex justify-between"><span style={{ color: C.muted }}>Harga Paket</span><span className="font-semibold" style={{ color: C.ink }}>{fmtIDR(pkg.price)}</span></div>
              <div className="flex justify-between"><span style={{ color: C.muted }}>Tambahan</span><span className="font-semibold" style={{ color: C.ink }}>{fmtIDR(addonsTotal)}</span></div>
              <div className="flex justify-between pt-1.5 mt-1" style={{ borderTop: `1px dashed ${C.line}` }}>
                <span className="font-bold" style={{ color: C.ink }}>Total Estimasi</span>
                <span className="font-bold" style={{ color: C.orange }}>{fmtIDR(grandTotal)}</span>
              </div>
            </div>
          )}
    ...
    <div className="rounded-2xl p-5 effect-3d-card" style={{ backgroundColor: C.paper, border: `1px solid ${C.line}` }}>
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          {b.addons && (b.addons.makeup || b.addons.photographer || b.addons.mc || b.addons.entertainment) && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {b.addons.makeup && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ backgroundColor: C.orangeSoft, color: C.orange }}>
                  <Icon name="Sparkles" size={10} /> MUA @{b.addons.makeup}
                </span>
              )}
              {b.addons.photographer && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ backgroundColor: C.greenSoft, color: C.green }}>
                  <Icon name="Camera" size={10} /> @{b.addons.photographer}
                </span>
              )}
              {b.addons.mc && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ backgroundColor: C.cyan, color: C.blueDeep }}>
                  <Icon name="Mic2" size={10} /> MC{b.addons.mc.vendor ? ` @${b.addons.mc.vendor}` : ""} ({b.addons.mc.type === "akad" ? "Akad" : b.addons.mc.type === "resepsi" ? "Resepsi" : "Akad+Resepsi"})
                </span>
              )}
              {b.addons.entertainment && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ backgroundColor: C.amberSoft, color: C.amber }}>
                  <Icon name="Music4" size={10} /> {b.addons.entertainment === "akustik" ? "Akustik" : "Orgen Tunggal"}
                </span>
              )}
            </div>
          )}
